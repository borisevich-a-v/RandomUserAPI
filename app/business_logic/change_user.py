...
def change_portrait(user_id, file):
    ...
    if file and allowed_file(file.filename):
        ...
        portrait_name = make_portraits(file, filename)
        user = User.query.filter(User.user_id == user_id).first()
        remove_pictures(
...
def next_portrait_file_name(dirpath):
    """Return max portrait file index or 0"""
    last_index = max(
        [
            int(os.path.splitext(filename)[0])
            for filename in (os.listdir(dirpath) or ["0.jpg"])
        ]
    )
    return str(last_index + 1) + ".jpg"


def resize_image(path):
    """Resize image and save it in file system and return their paths"""
    file_name = next_portrait_file_name(Path("app/static/portraits/large"))
    with Image.open(path) as img:
        img.thumbnail((128, 128))
        path_large = Path("app/static/portraits/large", file_name)
        img.save(path_large)

        img.thumbnail((48, 48))
        path_thumbnail = Path("app/static/portraits/thumbnail", file_name)
        img.save(path_thumbnail)
    return file_name


def make_portraits(file, filename):
